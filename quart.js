document.addEventListener('DOMContentLoaded', function () {
  const settings = {
    ...
    frequency: 3.0,
    amplitude: 6.0,
    intensity: 10.0, // Increased intensity for a brighter animation
    phase: 0.1,
  };
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      // Adjusted color values for brightness
      vec3 brightness = vec3(0.2, 0.2, 0.2); // Bright gray
      vec3 contrast = vec3(0.1, 0.2, 0.3);    // Adjusted for contrast
      vec3 oscilation = vec3(1.0, 1.0, 1.0);   // No change
  ...
  // Append the canvas to the element with class .sphere
  const canvas = renderer.domElement;
  canvas.classList.add('webgl');
  document.querySelector('.sphere').appendChild(canvas);
